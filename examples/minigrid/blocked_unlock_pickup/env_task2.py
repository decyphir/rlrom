...
class BlockedUnlockPickupKeyEnv(RoomGrid):
    """
    ...
    def _gen_grid(self, width, height):
        ...
        for _ in range(max_attempts):
            x = pos[0] - np.random.randint(1, 3)
            y = pos[1] - np.random.randint(-1, 2)
            
            # Validate before placing
            if 0 <= x < self.grid.width and 0 <= y < self.grid.height:
                if self.grid.get(x, y) is None:  # Also check if empty
                    self.grid.set(x, y, Ball(color))
                    break
...
register(
    id="MiniGrid-BlockedUnlockPickup-v3",
    entry_point=__name__ + ":BlockedUnlockPickupKeyEnv",
)