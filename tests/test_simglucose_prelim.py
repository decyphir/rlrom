...
with gr.Blocks(fill_height=True) as sg_gui:
    textbox_bg_specs = gr.Textbox(label="Specifications",lines=10, interactive=True)         
    ...
    with gr.Tabs():
        ...
        with gr.Tab(label="Plot"):
            with gr.Group():
                fig_bg = gr.Plot()
    button_bg_plot.click(update_plot, [textbox_bg_specs, textbox_bg_plot_prompt], [status_bg, fig])    
